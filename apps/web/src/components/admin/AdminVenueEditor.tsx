...
export function AdminVenueEditor({ venue, categories, isNew }: Props) {
  ...
  const { register, handleSubmit, getValues, setValue, formState: { errors, isSubmitting } } = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      city: venue?.city ?? 'Grand Rapids',
      state: venue?.state ?? 'MI',
      zip: venue?.zip ?? '',
      phone: venue?.phone ?? '',
      ...
      status: (venue?.status as FormValues['status']) ?? 'ACTIVE',
      verified: venue?.verified ?? false,
      premium: venue?.premium ?? false,
      autoApprove: venue?.autoApprove ?? false,
  ...
  async function handleDelete() {
    if (!venue) return
    if (!confirm(`Delete "${venue.name}" and all its deals? This cannot be undone.`)) return
    await deleteVenue.mutateAsync({ id: venue.id })
  ...
  async function handleRegenToken() {
    ...
    await regenToken.mutateAsync({ id: venue.id })
    router.refresh()
  }

  const portalUrl = venue
    ? `${process.env.NEXT_PUBLIC_APP_URL ?? ''}/venue/${venue.portalToken}`
    : null
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-text-primary">{isNew ? 'Add New Venue' : `Edit: ${venue?.name}`}</h1>
          {venue && (
            <p className="text-xs text-text-muted mt-0.5">{venue._count.deals} deals</p>
          )}
        </div>
        {!isNew && (
          <Button variant="danger" size="sm" icon={<Trash2 className="h-3.5 w-3.5" />}
            onClick={handleDelete} loading={deleteVenue.isPending}>
            Delete
          </Button>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <Section title="Basic Info">
          <Input label="Venue Name" required error={errors.name?.message} {...register('name')} />
        ...
        <Section title="Flags">
          <div className="space-y-3">
            <label className="flex items-center gap-2.5 cursor-pointer">
              ...
              <span className="text-sm font-medium text-text-primary">✓ Verified venue (shows badge)</span>
            </label>
            <label className="flex items-center gap-2.5 cursor-pointer">
              ...
              <span className="text-sm font-medium text-text-primary">⭐ Premium venue (priority placement)</span>
            </label>
            <label className="flex items-center gap-2.5 cursor-pointer">
              ...
              <span className="text-sm font-medium text-text-primary">Auto-approve portal submissions (trusted venue)</span>
            </label>
          </div>
        </Section>
        ...
        {(createVenue.error || updateVenue.error) && (
          <div className="rounded-lg bg-red-50 border border-red-200 p-3 text-sm text-brand-red">
            {createVenue.error?.message ?? updateVenue.error?.message}
          </div>
        )}

        <div className="flex gap-3 pt-2">
          <Button type="button" variant="secondary" onClick={() => router.back()}>Cancel</Button>
          <Button type="submit" loading={isSubmitting || createVenue.isPending || updateVenue.isPending}>
      ...
      {venue && portalUrl && (
        <div className="rounded-card border border-surface-border bg-white p-5">
          <h2 className="text-sm font-semibold text-text-primary mb-3">Venue Portal Link</h2>
          <p className="text-xs text-text-muted mb-3">
            Share this private link with the venue owner. They can manage their deals without a password.
          </p>
